...
loci = read_tsv(paste0(patient_dir,"/tables/loci.tsv"), col_types = cols())
cluster = read_tsv(paste0(patient_dir,"/tables/cluster.tsv"), col_types = cols())

# Filter out singleton clusters
cluster = cluster %>% filter(! size == 1)

top10count<-c(na.exclude(sort(unique(cluster$size),decreasing = T)[1:10]))
cluster =cluster%>%filter(size %in% top10count)
top10cluster<-c(na.exclude(sort(unique(cluster$cluster_id),decreasing = F)[1:10]))
cluster =cluster%>%filter(cluster_id %in% top10cluster)
