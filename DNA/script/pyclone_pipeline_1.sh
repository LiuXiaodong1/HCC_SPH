...
#PyClone run_analysis_pipeline --density pyclone_binomial --burnin ${burnin} --prior ${prior} --num_iters ${iters} --tumour_contents $(cat output/${sam}/purity.txt) --in_files $(ls ${sam}*.tsv) --working_dir output/${sam} --min_cluster_size 2 &> output/${sam}/setup_analysis.log 
